...
notificationQueue.process(async (job) => {
    ...
    const { processedFollowersKey, failedFollowersKey, totalFollowersKey } = getRedisKeys(postId);
    const totalFollowers = await redisClient.get(totalFollowersKey);
    console.log(`Processing batch of ${batch.length} followers for post ${postId}`);
    for (const follower of batch) {
        await new Promise(async (resolve) => {
            setTimeout(async () => {
                const isSuccess = Math.random() > 0.5;
                if (isSuccess) {
                    console.log(`Email sent to follower ${follower.id} for post ${postId}`);
                    await redisClient.incr(processedFollowersKey);
                }
                else {
                    console.log(`Failed to send email to follower ${follower.id} for post ${postId}`);
                    await redisClient.incr(failedFollowersKey);
                    failedNotificationQueue.add({ followerId: follower.id, postId, bloggerId }, {
                        attempts: 3,
    ...
    const totalCompleted = (parseInt(processedFollowers) || 0) + (parseInt(failedFollowers) || 0);
    if (totalCompleted >= parseInt(totalFollowers)) {
        notifyAuthor(bloggerId, postId, totalFollowers, processedFollowers, failedFollowers);
...
failedNotificationQueue.process(async (job) => {
    ...
    console.log(`Retrying email for failed follower ${followerId} for post ${postId}`);
    try {
        const isSuccess = true;
        if (isSuccess) {
            console.log(`Successfully sent email to follower ${followerId} on retry for post ${postId}`);
        }
        else {
        ...
        console.error(`Error processing follower ${followerId} for post ${postId}:`, err);
        throw err;
    }
});
//Helper function to notify the author
const notifyAuthor = async (bloggerId, postId, totalFollowers, processedFollowers, failedFollowers) => {
...
const setExpirationOnCompletion = async (postId) => {
    const { totalFollowersKey, processedFollowersKey, failedFollowersKey } = getRedisKeys(postId);
...
const sendNotifications = async (postId, bloggerId) => {
    const limit = 3;
    let offset = 0;
    try {
...
const getFollowers = async (bloggerId, limit, offset) => {
...
const getProgress = async (postId) => {
    const { totalFollowersKey, processedFollowersKey, failedFollowersKey } = getRedisKeys(postId);
    const totalFollowers = await redisClient.get(totalFollowersKey);
    const processedFollowers = await redisClient.get(processedFollowersKey);
    const failedFollowers = await redisClient.get(failedFollowersKey);
    if (totalFollowers && processedFollowers) {
        const progress = ((parseInt(processedFollowers) + parseInt(failedFollowers)) /
